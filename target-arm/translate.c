...
static inline void gen_aa32_ld##SUFF(TCGv_i32 val, TCGv_i32 addr, int index, TCGv_ptr env) \
{                                                                        \
    gen_helper_sync_point(env);                                          \
    tcg_gen_qemu_ld_i32(val, addr, index, OPC);                          \
}

#define DO_GEN_ST(SUFF, OPC)                                             \
static inline void gen_aa32_st##SUFF(TCGv_i32 val, TCGv_i32 addr, int index, TCGv_ptr env) \
{                                                                        \
    gen_helper_sync_point(env);                                          \
    tcg_gen_qemu_st_i32(val, addr, index, OPC);                          \
}

static inline void gen_aa32_ld64(TCGv_i64 val, TCGv_i32 addr, int index, TCGv_ptr env)
{
    gen_helper_sync_point(env);
    tcg_gen_qemu_ld_i64(val, addr, index, MO_TEQ);
}

static inline void gen_aa32_st64(TCGv_i64 val, TCGv_i32 addr, int index, TCGv_ptr env)
{
    gen_helper_sync_point(env);
...
{
    if (dp) {
        gen_aa32_ld64(cpu_F0d, addr, get_mem_index(s), cpu_env);
    } else {
        gen_aa32_ld32u(cpu_F0s, addr, get_mem_index(s), cpu_env);
    }
}
...
{
    if (dp) {
        gen_aa32_st64(cpu_F0d, addr, get_mem_index(s), cpu_env);
    } else {
        gen_aa32_st32(cpu_F0s, addr, get_mem_index(s), cpu_env);
    }
}
    ...
    if ((insn & 0x0e000e00) == 0x0c000000) {
        ...
        if (insn & ARM_CP_RW_BIT) {
            if ((insn >> 28) == 0xf) {			/* WLDRW wCx */
                tmp = tcg_temp_new_i32();
                gen_aa32_ld32u(tmp, addr, get_mem_index(s), cpu_env);
                iwmmxt_store_creg(wrd, tmp);
            } else {
                i = 1;
                if (insn & (1 << 8)) {
                    if (insn & (1 << 22)) {		/* WLDRD */
                        gen_aa32_ld64(cpu_M0, addr, get_mem_index(s), cpu_env);
                        i = 0;
                    } else {				/* WLDRW wRd */
                        tmp = tcg_temp_new_i32();
                        gen_aa32_ld32u(tmp, addr, get_mem_index(s), cpu_env);
                    ...
                    tmp = tcg_temp_new_i32();
                    if (insn & (1 << 22)) {		/* WLDRH */
                        gen_aa32_ld16u(tmp, addr, get_mem_index(s), cpu_env);
                    } else {				/* WLDRB */
                        gen_aa32_ld8u(tmp, addr, get_mem_index(s), cpu_env);
                    }
                }
                if (i) {
            ...
            if ((insn >> 28) == 0xf) {			/* WSTRW wCx */
                ...
                gen_aa32_st32(tmp, addr, get_mem_index(s), cpu_env);
            } else {
                gen_op_iwmmxt_movq_M0_wRn(wrd);
                tmp = tcg_temp_new_i32();
                if (insn & (1 << 8)) {
                    if (insn & (1 << 22)) {		/* WSTRD */
                        gen_aa32_st64(cpu_M0, addr, get_mem_index(s), cpu_env);
                    } else {				/* WSTRW wRd */
                    ...
                    if (insn & (1 << 22)) {		/* WSTRH */
                        tcg_gen_trunc_i64_i32(tmp, cpu_M0);
                        gen_aa32_st16(tmp, addr, get_mem_index(s), cpu_env);
                    } else {				/* WSTRB */
                        tcg_gen_trunc_i64_i32(tmp, cpu_M0);
                        gen_aa32_st8(tmp, addr, get_mem_index(s), cpu_env);
                    }
                }
            }
            tcg_temp_free_i32(tmp);
    ...
    if ((insn & (1 << 23)) == 0) {
        ...
        for (reg = 0; reg < nregs; reg++) {
            ...
            if (size == 3) {
                ...
                if (load) {
                    gen_aa32_ld64(tmp64, addr, get_mem_index(s), cpu_env);
                    neon_store_reg64(tmp64, rd);
                } else {
                    neon_load_reg64(tmp64, rd);
                    gen_aa32_st64(tmp64, addr, get_mem_index(s), cpu_env);
                }
                tcg_temp_free_i64(tmp64);
                ...
                for (pass = 0; pass < 2; pass++) {
                    if (size == 2) {
                        ...
                        if (load) {
                            ...
                            tcg_gen_addi_i32(addr, addr, stride);
                            tmp2 = tcg_temp_new_i32();
                            gen_aa32_ld16u(tmp2, addr, get_mem_index(s), cpu_env);
                            tcg_gen_addi_i32(addr, addr, stride);
                            tcg_gen_shli_i32(tmp2, tmp2, 16);
                            tcg_gen_or_i32(tmp, tmp, tmp2);
                            tcg_temp_free_i32(tmp2);
                            ...
                            tcg_temp_free_i32(tmp);
                            tcg_gen_addi_i32(addr, addr, stride);
                            gen_aa32_st16(tmp2, addr, get_mem_index(s), cpu_env);
                            tcg_temp_free_i32(tmp2);
                            tcg_gen_addi_i32(addr, addr, stride);
                        }
                    } else /* size == 0 */ {
                               ...
                               TCGv_i32 addr, int size)
{
    ...
    if (size == 3) {
        ...
        gen_aa32_ld32u(tmp3, tmp2, get_mem_index(s), cpu_env);
        tcg_temp_free_i32(tmp2);
...
static void disas_arm_insn(DisasContext *s, unsigned int insn)
{
    ...
    if (cond == 0xf){
        ...
        if ((insn & 0x0ffffdff) == 0x01010000) {
            ...
            tcg_gen_addi_i32(addr, addr, 4);
            tmp2 = tcg_temp_new_i32();
            gen_aa32_ld32u(tmp2, addr, get_mem_index(s), cpu_env);
            if (insn & (1 << 21)) {
                /* Base writeback.  */
                switch (i) {
    ...
    if ((insn & 0x0f900000) == 0x03000000) {
            ...
            if (sh == 0) {
                if (op1 == 0x0) {
                    ...
                    if (insn & (1 << 23)) {
                        ...
                        tmp2 = tcg_temp_new_i32();
                        if (insn & (1 << 22)) {
                            gen_aa32_ld8u(tmp2, addr, get_mem_index(s), cpu_env);
                            gen_aa32_st8(tmp, addr, get_mem_index(s), cpu_env);
                        } else {
                            gen_aa32_ld32u(tmp2, addr, get_mem_index(s), cpu_env);
                            gen_aa32_st32(tmp, addr, get_mem_index(s), cpu_env);
                ...
                if (insn & (1 << 20)) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        gen_aa32_ld8s(tmp, addr, get_mem_index(s), cpu_env);
                        break;
                    default:
                    case 3:
                        gen_aa32_ld16s(tmp, addr, get_mem_index(s), cpu_env);
                        break;
                    }
                    load = 1;
                } else if (sh & 2) {
        ...
        do_ldst:
            ...
            if (insn & (1 << 20)) {
                ...
                tmp = tcg_temp_new_i32();
                if (insn & (1 << 22)) {
                    gen_aa32_ld8u(tmp, tmp2, i, cpu_env);
                } else {
                    gen_aa32_ld32u(tmp, tmp2, i, cpu_env);
                }
            } else {
                /* store */
                tmp = load_reg(s, rd);
                if (insn & (1 << 22)) {
                    gen_aa32_st8(tmp, tmp2, i, cpu_env);
                } else {
                    gen_aa32_st32(tmp, tmp2, i, cpu_env);
                }
                tcg_temp_free_i32(tmp);
            }
            if (!(insn & (1 << 24))) {
...
static int disas_thumb2_insn(CPUARMState *env, DisasContext *s, uint16_t insn_hw1)
{
    ...
    case 12: /* Load/store single data item.  */
        ...
        if (insn & (1 << 20)) {
            ...
            switch (op) {
            case 0:
                gen_aa32_ld8u(tmp, addr, memidx, cpu_env);
                break;
            case 4:
                gen_aa32_ld8s(tmp, addr, memidx, cpu_env);
                break;
            case 1:
                gen_aa32_ld16u(tmp, addr, memidx, cpu_env);
                break;
            case 5:
                gen_aa32_ld16s(tmp, addr, memidx, cpu_env);
                break;
            case 2:
                gen_aa32_ld32u(tmp, addr, memidx, cpu_env);
                break;
            default:
                tcg_temp_free_i32(tmp);
                tcg_temp_free_i32(addr);
            ...
            switch (op) {
            case 0:
                gen_aa32_st8(tmp, addr, memidx, cpu_env);
                break;
            case 1:
                gen_aa32_st16(tmp, addr, memidx, cpu_env);
                break;
            case 2:
                gen_aa32_st32(tmp, addr, memidx, cpu_env);
                break;
            default:
                tcg_temp_free_i32(tmp);
                tcg_temp_free_i32(addr);
